...
def _session() -> requests.Session:
    # Con reintentos + backoff: un 503 puntual ya no tumba la búsqueda
    return net.make_session(HEADERS_HTTP)
...
class AnimeFLVScraper(BaseScraper):
    ...
    def buscar(self, nombre: str) -> Optional[AnimeData]:
        ...
        try:
            ...
            # Episodios.
            # Por defecto "?" (desconocido), NUNCA "película": marcar película
            # solo cuando la fuente lo indica explícitamente (Tipo: Película).
            # Antes el default era "película", así que cualquier serie cuyo
            # bloque de episodios no se parseara quedaba mal marcada como peli.
            capitulos: int | str = "?"
            es_pelicula = False
            ...
            if tipo_el:
                ...
                if parent:
                    ...
                    if "película" in tipo_text or "pelicula" in tipo_text or "movie" in tipo_text:
                        es_pelicula = True
            ...
            # Si no encontramos número de episodios, contar los listados en la página
            if capitulos == "?":
                ep_links = soup.select("ul.ListCaps li")
                if ep_links:
                    capitulos = len(ep_links)

            # Película: solo si la fuente lo marca y no hay un recuento real (>1)
            if es_pelicula and capitulos in ("?", 1):
                capitulos = "película"
